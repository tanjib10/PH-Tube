...
const loadCategory = async (category) => {
  ...
  data.data.forEach((category) => {
    ...
    div.innerHTML = `
      <button onclick="categoryData('${category.category_id}')" class="btn mr-4 focus:bg-red-500 focus:text-white">${category.category}</button>
    `;
...
const categoryData = async (id) => {
  ...
  if (data.data.length === 0) {
    ...
    categoryDataArray = data.data;
    loadData(categoryDataArray); 
  }
};
/* sort by views */
const sortByViews = () => {
  categoryDataArray.sort((a, b) => parseFloat(b.others.views) - parseFloat(a.others.views));
  loadData(categoryDataArray);
};

const loadData = (dataArray) => {
  const cardContainer = document.getElementById('card-container');
  cardContainer.innerHTML = '';
  ...
  dataArray.forEach((id) => {
    ...
    div.innerHTML = `
      <div class="card w-70 hover:cursor-pointer">
        <div class="relative">
          <img src="${id.thumbnail}" alt="" class="w-full rounded-lg h-44" />
        </div>
...
/* second to hour and minutes function */
